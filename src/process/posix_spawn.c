...
{
#ifdef SYS_dup2
	return __syscall_SYS_dup2(old, new);
#else
	return __syscall_SYS_dup3(old, new, 0);
#endif
}
...
static int child(void *args_vp)
{
	...
	if (attr->__flags & POSIX_SPAWN_SETSID)
		if ((ret=__syscall_SYS_setsid()) < 0)
			goto fail;

	if (attr->__flags & POSIX_SPAWN_SETPGROUP)
		if ((ret=__syscall_SYS_setpgid(0, attr->__pgrp)))
			goto fail;
	...
	if (attr->__flags & POSIX_SPAWN_RESETIDS)
		if ((ret=__syscall_SYS_setgid(__syscall_SYS_getgid())) ||
		    (ret=__syscall_SYS_setuid(__syscall_SYS_getuid())) )
			goto fail;
	...
	if (fa && fa->__actions) {
		...
		for (; op; op = op->prev) {
			 ...
			 * an unoccupied fd. */
			if (op->fd == p) {
				ret = __syscall_SYS_dup(p);
				if (ret < 0) goto fail;
				__syscall_SYS_close(p);
				p = ret;
			}
			switch(op->cmd) {
			case FDOP_CLOSE:
				__syscall_SYS_close(op->fd);
				break;
			case FDOP_DUP2:
				fd = op->srcfd;
				if (fd == p) {
			...
			case FDOP_OPEN:
				...
				if (fd != op->fd) {
					if ((ret=__sys_dup2(fd, op->fd))<0)
						goto fail;
					__syscall_SYS_close(fd);
				}
				break;
			case FDOP_CHDIR:
				ret = __syscall_SYS_chdir(op->path);
				if (ret<0) goto fail;
				break;
			case FDOP_FCHDIR:
				ret = __syscall_SYS_fchdir(op->fd);
				if (ret<0) goto fail;
				break;
			}
		}
	...
	__syscall_SYS_fcntl(p, F_SETFD, FD_CLOEXEC);
...
fail:
	...
	ret = -ret;
	if (ret) while (__syscall_SYS_write(p, &ret, sizeof ret) < 0);
	_exit(127);
}
